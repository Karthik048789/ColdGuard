...
if __name__ == '__main__':
    all_endpoints = [
        ...
        # Facility Module
        ["Facility", "GET", "/api/facilities", "Bearer Token (auth:sanctum)", "List all cold-storage facilities, optionally filtered by status (AVAILABLE/UNAVAILABLE)", "Query Params:\nstatus=AVAILABLE", "{\n  \"success\": true,\n  \"data\": [\n    {\n      \"id\": 1,\n      \"name\": \"ColdGuard Kakkanad Storage B (Demo)\",\n      \"latitude\": 10.0250,\n      \"longitude\": 76.3500,\n      \"capacity\": 1500,\n      \"available_capacity\": 1200,\n      \"min_temperature\": 2.0,\n      \"max_temperature\": 8.0,\n      \"cost\": 650.00,\n      \"status\": \"AVAILABLE\"\n    }\n  ]\n}"],
        ["Facility", "GET", "/api/shipments/{id}/facilities/eligible", "Bearer Token (auth:sanctum)", "Retrieve eligible cold-storage facilities ranked by approximate distance from truck current GPS", "Path Param:\nid=1 or CG-2026-VACC-001", "{\n  \"success\": true,\n  \"message\": \"Eligible facilities retrieved and ranked successfully.\",\n  \"data\": {\n    \"shipment\": { \"id\": 1, \"tracking_number\": \"CG-2026-VACC-001\", \"required_temperature\": {\"min\": 2, \"max\": 8}, \"quantity\": 500 },\n    \"current_location\": { \"latitude\": 10.0159, \"longitude\": 76.3419 },\n    \"facilities\": [ {\"id\": 3, \"name\": \"Aluva Med-Hub Emergency Cold Store\", \"approximate_distance_km\": 1.4, ...} ],\n    \"recommended_facility\": { \"id\": 3, \"name\": \"...\", \"reason\": \"Eligible temperature range, sufficient capacity...\" }\n  }\n}"],

        # Routing & Live Tracking Module
        ["Routing", "GET", "/api/shipments/{id}/location", "Bearer Token (auth:sanctum)", "Retrieve latest live truck GPS location and sensor telemetry for continuous tracking", "Path Param:\nid=1 or CG-2026-VACC-001", "{\n  \"success\": true,\n  \"data\": {\n    \"shipment_id\": 1,\n    \"tracking_number\": \"CG-2026-VACC-001\",\n    \"product_name\": \"Measles-Rubella (MR) Vaccine\",\n    \"location\": { \"latitude\": 10.16976, \"longitude\": 76.24614 },\n    \"temperature\": 7.8,\n    \"humidity\": 64.0,\n    \"battery\": 81.0,\n    \"recorded_at\": \"2026-09-12T12:30:00Z\"\n  }\n}"],
        ["Routing", "GET", "/api/shipments/{id}/route", "Bearer Token (auth:sanctum)", "Calculate OSRM multi-waypoint road route (Truck -> Facility -> Receiver) with GeoJSON geometry, distance, ETA, and turn-by-turn steps", "Path Param:\nid=1 or CG-2026-VACC-001\nQuery Params:\nfacility_id=3", "{\n  \"success\": true,\n  \"message\": \"Road route calculated successfully.\",\n  \"data\": {\n    \"shipment\": { \"id\": 1, \"tracking_number\": \"CG-2026-VACC-001\" },\n    \"current_location\": { \"latitude\": 10.16976, \"longitude\": 76.24614 },\n    \"facility\": { \"id\": 3, \"name\": \"Aluva Med-Hub Emergency Cold Store\", \"latitude\": 10.1004, \"longitude\": 76.3570 },\n    \"destination\": { \"name\": \"Thrissur District Hospital\", \"latitude\": 10.5276, \"longitude\": 76.2144 },\n    \"distance_km\": 78.03,\n    \"duration_minutes\": 80,\n    \"geometry\": { \"type\": \"LineString\", \"coordinates\": [[76.24614, 10.16976], ...] },\n    \"legs\": [...],\n    \"steps\": [...]\n  }\n}"],

        # Intervention & Alerts Module
        ["Intervention", "GET", "/api/shipments/{id}/intervention", "Bearer Token (auth:sanctum)", "Retrieve current active intervention for a shipment (facility, risk, OSRM route, recipient alerts)", "Path Param:\nid=1 or CG-2026-VACC-001", "{\n  \"success\": true,\n  \"data\": {\n    \"id\": 1,\n    \"status\": \"DIVERTED\",\n    \"severity\": \"CRITICAL\",\n    \"risk_score\": 98.0,\n    \"reason\": \"CRITICAL TEMPERATURE BREACH...\",\n    \"selected_facility\": { \"id\": 3, \"name\": \"Aluva Med-Hub Emergency Cold Store\" },\n    \"route\": { \"distance_km\": 81.47, \"duration_minutes\": 82 },\n    \"alerts\": [...]\n  }\n}"],
        ["Intervention", "GET", "/api/shipments/{id}/interventions", "Bearer Token (auth:sanctum)", "Retrieve complete historical interventions list for a shipment", "Path Param:\nid=1 or CG-2026-VACC-001", "{\n  \"success\": true,\n  \"data\": [ {\"id\": 1, \"status\": \"RESOLVED\", \"severity\": \"CRITICAL\", \"resolved_at\": \"ISO-8601\", ...} ]\n}"],
        ["Intervention", "GET", "/api/shipments/{id}/alerts", "Bearer Token (auth:sanctum)", "Retrieve alerts for shipment (supports role filter: MANAGER|DRIVER|RECEIVER)", "Path Param:\nid=1 or CG-2026-VACC-001\nQuery Params:\nrole=DRIVER", "{\n  \"success\": true,\n  \"data\": [ {\"id\": 1, \"recipient_role\": \"DRIVER\", \"type\": \"DIVERSION_REQUIRED\", \"title\": \"...\", \"message\": \"...\"} ]\n}"],
        ["Intervention", "POST", "/api/interventions/{id}/acknowledge", "Bearer Token (auth:sanctum)", "Acknowledge / mark alerts for an intervention as read", "Path Param:\nid=1", "{\n  \"success\": true,\n  \"message\": \"Alerts for intervention 1 acknowledged successfully.\"\n}"],
        ["Intervention", "POST", "/api/interventions/{id}/resolve", "Bearer Token (auth:sanctum)", "Manually resolve an active intervention", "Path Param:\nid=1", "{\n  \"success\": true,\n  \"message\": \"Intervention resolved successfully.\",\n  \"intervention\": { \"id\": 1, \"status\": \"RESOLVED\", \"resolved_at\": \"ISO-8601\" }\n}"]
    ]
